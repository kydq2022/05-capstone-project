...
import { deleteAttachment } from '../fileStorage/attachmentUtils'
...
export const deleteTodo =async ( todoId: string, userId: string) => {
    try {
        ...
        // Delete todo attachment image
        logger.info(todo);
        const key = todo.attachmentUrl?.split('/').pop();
        logger.info(`delete image ${key}`);
